...
Import the pre-built dashboard in Grafana via **Dashboards → Import → Upload dashboard JSON file** using [`groster-dashboard.json`](groster-dashboard.json).

During import, Grafana will prompt you to select a Loki data source — pick the one configured in Step 6.
...
| **Top 10 users**               | Table of Discord users who invoked slash commands, ranked by query count.                                      |
| **Top 10 searched characters** | Table of WoW characters looked up via /whois, ranked by lookup count.                                          |
